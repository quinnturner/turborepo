...
// ReadLockfile will read `yarn.lock` into memory (either from the cache or fresh)
func ReadBerryLockfile(rootpath string, backendName string, cacheDir AbsolutePath) (*BerryLockfile, error) {
	var lockfile BerryLockfile
	var prettyLockFile = BerryLockfile{}
	hash, err := HashFile(filepath.Join(rootpath, "yarn.lock"))
	if err != nil {
		return &BerryLockfile{}, fmt.Errorf("failed to hash lockfile: %w", err)
	}
	turboLockFile := cacheDir.Join(fmt.Sprintf("%v-turbo-berry-lock.yaml", hash))
	contentsOfLock, err := turboLockFile.ReadFile()
	if err != nil {
		contentsB, err := ioutil.ReadFile(filepath.Join(rootpath, "yarn.lock"))
		if err != nil {
		...
		if contentsOfLock != nil {
			err = yaml.Unmarshal(contentsOfLock, &prettyLockFile)
			if err != nil {
				return &BerryLockfile{}, fmt.Errorf("could not unmarshal yaml: %w", err)
			}
		}
	}
